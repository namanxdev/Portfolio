...
const GLSLHills = ({ width = '100%', height = '100%', cameraZ = 125, planeSize = 256, speed = 0.5 }) => {
  ...
  useEffect(() => {
        ...
        return new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            fragmentShader: `
              ...
              void main(void) {
                // Brighten the waves and expand the visible radius
                float opacity = (128.0 - length(vPosition)) / 256.0 * 0.75;
                // Create a sleek blue/purple gradient color based on position
                vec3 color = vec3(0.2, 0.45, 1.0) * (vPosition.z * 0.02) + vec3(0.5, 0.25, 0.95);
    ...
    // We get real width/height of parent container
    const widthVal = containerRef.current?.clientWidth || window.innerWidth;
    const heightVal = containerRef.current?.clientHeight || window.innerHeight;
    
    const camera = new THREE.PerspectiveCamera(45, widthVal / heightVal, 1, 10000);
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-hidden w-full h-full">
      <canvas
        ref={canvasRef}
        className="absolute top-0 right-0 bottom-0 left-0 z-0 pointer-events-none"
      />
    </div>
  );
};
